...
namespace Pg.LetsMeet.Dataverse.Plugins.Core
{
    public abstract class PluginBase : IPlugin 
    {
        ...
        public abstract IDependencyLoader DependencyLoader { get; set; }
        ...
        protected static TEntity GetPreImageEntity<TEntity>(IPluginExecutionContext context, string imageName) where TEntity : Entity
        {
            return GetPreImageEntity(context, imageName)?.ToEntity<TEntity>();
        }

        protected static TEntity GetPostImageEntity<TEntity>(IPluginExecutionContext context, string imageName) where TEntity : Entity
        {
            return GetPostImageEntity(context, imageName)?.ToEntity<TEntity>();
        }

        protected static Entity GetPreImageEntity(IPluginExecutionContext context, string imageName)
        {
            return context != null && context.PreEntityImages != null && context.PreEntityImages.Contains(imageName)
             ? context.PreEntityImages[imageName] as Entity
             : null;
        }

        protected static Entity GetPostImageEntity(IPluginExecutionContext context, string imageName)
        {
            return context != null && context.PostEntityImages != null && context.PostEntityImages.Contains(imageName)
             ? context.PostEntityImages[imageName] as Entity
             : null;
        }
